const menuBtn = document.querySelector('.menu-btn');
    ...
    function displayData(bookings) {
        ...
        bookings.forEach(booking => {
            ...
            <tr>
                ...
                <td>
                <button class="btn btn-edit btn-sm" onclick="cancelRow(${booking.booking_id})">Hủy</button>
                <button class="btn btn-delete btn-sm" onclick="deleteRow(${booking.booking_id})">Xóa</button>
                <button class="btn btn-delete btn-sm" onclick="issuanceRow(${booking.booking_id})">Xuất vé</button>
                </td>
            </tr>
    ...
    //         booking_time: document.getElementById('booking_time').value.trim(),
    ...
    //     fetch('http://192.168.60.5:8000/api/bookings', {
    //         method: 'POST',
    //         headers: {
    //             'Content-Type': 'application/json',
    //             'Authorization': `Bearer ${authToken}`
    ...
    //             if (!response.ok) throw new Error('Failed to insert booking');
    //             return response.json();
    //         })
    //         .then(data => {
    ...
    //     const bookingId = window.currentEditingbookingId; // ID Vé đang chỉnh sửa
    //     if (!bookingId) {
    ...
    //     fetch(`http://192.168.60.5:8000/api/bookings/${bookingId}`, {
    //         method: 'PUT',
    //         headers: {
    //             'Content-Type': 'application/json',
    //             'Authorization': `Bearer ${authToken}`
    ...
    function deleteRow(bookingId) {
        if (!confirm(`Bạn có chắc chắn muốn xóa Vé với ID ${bookingId}?`)) {
            return;
        }
        ...
        fetch(`http://192.168.60.5:8000/api/bookings/${bookingId}`, {
            ...
            .then(() => {
                alert(`Xóa Vé với ID ${bookingId} thành công!`);
                loadData(1); // Tải lại danh sách Vé
            })
            .catch(error => {
    ...
    function cancelRow(bookingId) {
        if (!confirm(`Bạn có chắc chắn muốn hủy Vé với ID ${bookingId}?`)) {
            return;
        }
